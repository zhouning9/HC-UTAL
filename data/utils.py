...
def get_proposal_dict_um(cas_pred, aness_pred, feat_magnitudes_np, pred, score_np, vid_num_seg, config):
    prop_dict = {}
    for th in config.CAS_THRESH:
        cas_tmp = cas_pred.copy()
        num_segments = cas_pred.shape[0]//config.UP_SCALE
    ...
    for th in config.MAGNITUDES_THRESH:
        cas_tmp = cas_pred.copy()
        num_segments = cas_pred.shape[0]//config.UP_SCALE
        feat_magnitudes_np_temp = feat_magnitudes_np.copy()
        feat_magnitudes_np_temp[feat_magnitudes_np_temp[:, :, 0] < th] = 0
        seg_list = [np.where(feat_magnitudes_np_temp[:, c, 0] > 0)
...
def table_format(res_info, tIoU_thresh, title):
    table = [
        ...
        ['{:.4f}'.format(res_info['mAP@{:.1f}'.format(i)][-1])
         for i in tIoU_thresh]
    ]
    ...
    for i in range(len(tList)):
        ...
        if temp_list.any():
            ...
            for j in range(len(grouped_temp_list)):
                ...
                outer_s = max(
                    0, int(grouped_temp_list[j][0] - _lambda * len_proposal))
                outer_e = min(
                    int(wtcam.shape[0] - 1), int(grouped_temp_list[j][-1] + _lambda * len_proposal))
...
def save_best_record_thumos(test_info, file_path):
    ...
    for i in range(len(tIoU_thresh)):
        fo.write("mAP@{:.1f}: {:.4f}\n".format(
            tIoU_thresh[i], test_info["mAP@{:.1f}".format(tIoU_thresh[i])][-1]))
    fo.close()
...
def set_path(config):
    if config.MODE == 'train':
        config.EXP_NAME = 'us/noboundary_{cfg.R_noboundary}_boundary_{cfg.R_boundary}' \
            '_m_{cfg.m}_M_{cfg.M}_freq_{cfg.TEST_FREQ}' \
            '_seed_{cfg.SEED}_rp=9'.format(cfg=config)
        # config.EXP_NAME = 'experiments/cluster_freq_{cfg.TEST_FREQ}_instance_temperature_{cfg.instance_temperature}_cluster_temperature_{cfg.cluster_temperature}' \
        #        '_seed_{cfg.SEED}_half'.format(cfg=config)
